...
#        Neu ca hai cÛ trong file actor_movies.txt, tiep tuc;
#        nguoc lai, thoat
...
read usr_option
case "$usr_option" in
"A" ) echo "So dien vien co dong 1 trong hai film tren"
egrep -i "$movie1|$movie2" $1 | cut -f 1 -d',';;
"a" ) echo "So dien vien co dong 1 trong hai film tren"
egrep -i "$movie1|$movie2" $1 | cut -f 1 -d',';;
"C" ) echo "So dien vien co dong chung ca hai film tren"
grep -i "$movie1.*$movie2" $1 | cut -f 1 -d',';;
"c" ) echo "So ddien vien co dong chung ca hai film tren"
grep -i "$movie1.*$movie2" $1 | cut -f 1 -d',';;
"O" ) echo "So dien vien chi dong 1 film"
grep -vf actors_in_movie2.txt actors_in_movie1.txt
grep -vf actors_in_movie1.txt actors_in_movie2.txt;;
"o" ) echo "So dien vien chi dong 1 film"
grep -vf actors_in_movie2.txt actors_in_movie1.txt
grep -vf actors_in_movie1.txt actors_in_movie2.txt;;
"Q" ) exit 0;;
